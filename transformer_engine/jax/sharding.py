...
class FMHAShardingMetaGenerator(ShardingMetaGenerator):
    ...
    def get_dp_sharding_meta(
        ...
        return self._get_dptp_sharding_meta(input_shapes, output_shapes, dp_dims, dummy_tp_dims, dp_axis_name, None)

    def get_tp_col_sharding_meta(self, *argv, **kwargs) -> ShardingMeta:
        """get_tp_col_sharding_meta"""
        return self._get_tp_sharding_meta(*argv, **kwargs)
    ...
    def get_dp_tp_col_sharding_meta(self, *argv, **kwargs) -> ShardingMeta:
        """get_dp_tp_col_sharding_meta"""
        return self._get_dptp_sharding_meta(*argv, **kwargs)
    ...
    def _get_tp_sharding_meta(
        ...
        return self._get_dptp_sharding_meta(input_shapes, output_shapes, dummy_dp_dims, tp_dims, None, tp_axis_name)

    def _get_dptp_sharding_meta(
        self,
        input_shapes: Tuple[Tuple[int, ...]],
        output_shapes: Tuple[Tuple[int, ...]],
        dp_dims: Tuple[Tuple[int, ...]],
        ...
        for input_shape, dp_dim, tp_dim in zip(input_shapes, input_dp_dims, input_tp_dims):
            ...
            if dp_dim is not None:
                ...
                assert input_shape[dp_dim] % dp_size == 0, \
                    f"The dimension of batch in input_shape should be a multiple of data parallelism " \
                    f"size, but got {input_shape[dp_dim]=} and {dp_size=}."
                input_shape = (*input_shape[:dp_dim], dp_size, input_shape[dp_dim] // dp_size, *input_shape[dp_dim + 1:])
            ...
            if tp_dim is not None:
                ...
                assert input_shape[tp_dim] % tp_size == 0, \
                    f"The dimension of tensor parallel in input_shape should be a multiple of data " \
                    f"parallelism size, but got {input_shape[tp_dim]=} and {tp_size=}."
                input_shape = (*input_shape[:tp_dim], tp_size, input_shape[tp_dim] // tp_size, *input_shape[tp_dim + 1:])
        ...
        return ShardingMeta(tuple(in_axes), out_axes, axis_resources, input_new_shapes, output_new_shapes)
...
def get_fmha_sharding_meta(stype: ShardingType,
    ...
    """
    return FMHAShardingMetaGenerator().get_sharding_meta(stype, input_shapes,
        output_shapes, dp_dims, tp_dims, dp_axis_name, tp_axis_name)
