...
from transformer_engine.jax.mha import self_mha, cross_mha
...
def make_causal_mask(x: Array, extra_batch_dims: int = 0) -> Array:
    ...
    return make_attention_mask(idxs,
                               idxs,
                               jnp.greater_equal,
...
def make_decoder_mask(decoder_target_tokens: Array,
    ...
    if decoder_causal_attention is not None:
        # [batch, 1, length, length]
        inputs_mask = make_attention_mask(decoder_causal_attention,
    ...
    # Padding mask.
    masks.append(
        make_attention_mask(decoder_target_tokens > 0,
                            decoder_target_tokens > 0))

    # Packing mask
    if decoder_segment_ids is not None:
        masks.append(
            make_attention_mask(decoder_segment_ids, decoder_segment_ids,
                                jnp.equal))
...
def jax_fmha_encoder(qkv, bias, q_token, kv_token, **kwargs):
    ...
    # key = key * kwargs['scaling_factor']  # TODO(rewang): change to query
...
def jax_cross_mha(q, kv, q_token, kv_token, **kwargs):
    is_causal_masking = kwargs['is_causal_masking']
    if is_causal_masking:
...
def customcall_self_fmha(qkv, bias, q_token, kv_token, **kwargs):
    ...
    return self_mha(qkv, bias, q_seqlen, kv_seqlen, **kwargs)


def customcall_cross_mha(q, kv, q_token, kv_token, **kwargs):
    is_causal_masking = kwargs['is_causal_masking']
    if is_causal_masking:
        raise NotImplementedError
    mask = make_attention_mask(q_token > 0, kv_token > 0)
    ...
    return cross_mha(q, kv, q_seqlen, kv_seqlen, **kwargs)
...
class TestSelfFMHA():
    ...
    def set_input(self, b, s, h, d, dtype, is_causal_masking, pad_len):
        ...
        min_val, max_val = -1, 1
        self.qkv = jax.random.uniform(subkeys[0], qkv_shape, dtype, min_val,
                                      max_val)
        self.bias = jax.random.uniform(subkeys[1], bias_shape, dtype, min_val,
                                       max_val)

        self.q_token = jnp.concatenate((jnp.ones(
            (b, self.valid_len)), jnp.zeros((b, pad_len))),
                                       axis=-1)
    ...
    def test_forward(self, b, s, h, d, is_causal_masking, dtype):
        ...
        reference_out = jax_fmha_encoder(
            self.qkv,
            self.bias,
            self.q_token,
            self.kv_token,
        ...
        primitive_out = customcall_self_fmha(
            self.qkv,
            self.bias,
            self.q_token,
            self.kv_token,
        ...
        pri_valid, pri_invalid = jnp.split(primitive_out, (self.valid_len, ),
                                           axis=1)
    ...
    def test_forward_backward(self, b, s, h, d, is_causal_masking, dtype):
        ...
        def grad_func(fmha_func, *args, **kwargs):
            ...
            valid_fmha_ret, _ = jnp.split(fmha_func(*args, **kwargs),
                                          (self.valid_len, ),
                                          axis=1)
            return (jnp.mean(valid_fmha_ret, dtype=jnp.float32) *
        ...
        jitted_primitive = jit(
            value_and_grad(
                lambda qkv, bias, q_token, kv_token: grad_func(
                    customcall_self_fmha, qkv, bias, q_token, kv_token, **
                    kwargs), (0, 1)))

        jitted_reference = jit(
            value_and_grad(
                lambda qkv, bias, q_token, kv_token: grad_func(
                    jax_fmha_encoder, qkv.astype(jnp.float32),
                    bias.astype(jnp.float32), q_token, kv_token, **kwargs),
                (0, 1)))

        primitive_out, (primitive_dqkv, primitive_dbeta) = jitted_primitive(
            self.qkv, self.bias, self.q_token, self.kv_token)

        reference_out, (reference_dqkv, reference_dbeta) = jitted_reference(
            self.qkv, self.bias, self.q_token, self.kv_token)
        ...
        valid_primitive_dqkv, invalid_primitive_dqkv = jnp.split(
            primitive_dqkv, (self.valid_len, ), axis=1)
        valid_reference_dqkv, invalid_reference_dqkv = jnp.split(
            reference_dqkv, (self.valid_len, ), axis=1)

        # TODO(rewang): check dim
        # dQ
        np.testing.assert_allclose(
            jnp.asarray(valid_primitive_dqkv[:, :, 0, :], np.float32),
        ...
        # dK
        np.testing.assert_allclose(
            jnp.asarray(valid_primitive_dqkv[:, :, 1, :], np.float32),
        ...
        # dV
        np.testing.assert_allclose(
            jnp.asarray(valid_primitive_dqkv[:, :, 2, :], np.float32),
        ...
        assert jnp.allclose(invalid_primitive_dqkv,
                            jnp.zeros_like(invalid_primitive_dqkv))
        ...
        np.testing.assert_allclose(
            jnp.asarray(
                ...
                np.float32),
            jnp.asarray(
                reference_dbeta[:, :, :self.valid_len, :self.valid_len],
                np.float32),
            rtol=1e-4,
            atol=1e-5)
        ...
        assert jnp.allclose(
            primitive_dbeta[:, :, self.valid_len:, self.valid_len:],
            jnp.zeros_like(primitive_dbeta[:, :, self.valid_len:,
                                           self.valid_len:]))
...
class TestCrossFMHA():
    ...
    def set_input(self, b, s_q, s_kv, h, d, dtype, is_causal_masking, pad_len):
        ...
        min_val, max_val = -1, 1
        self.q = jax.random.uniform(subkeys[0], q_shape, dtype, min_val,
                                    max_val)
        self.kv = jax.random.uniform(subkeys[1], kv_shape, dtype, min_val,
                                     max_val)

        self.q_token = jnp.concatenate((jnp.ones(
            (b, self.q_valid_len)), jnp.zeros((b, pad_len))),
                                       axis=-1)
        self.kv_token = jnp.concatenate((jnp.ones(
            (b, self.kv_valid_len)), jnp.zeros((b, pad_len))),
                                        axis=-1)
        self.seed = 0
        self.scaling_factor = 1. / math.sqrt(d)
        self.dropout_probability = 0.
    ...
    def test_forward(self, b, s_q, s_kv, h, d, is_causal_masking, dtype):
        ...
        primitive_out = customcall_cross_mha(
            self.q,
            self.kv,
            self.q_token,
            self.kv_token,
        ...
        pri_valid, pri_invalid = jnp.split(primitive_out, (self.q_valid_len, ),
                                           axis=1)
    ...
    @pytest.mark.parametrize('is_causal_masking', [False])
    @pytest.mark.parametrize('dtype', DTYPES)
    def test_forward_backward(self, b, s_q, s_kv, h, d, is_causal_masking,
                              dtype):
        self.set_input(b,
                       s_q,
                       s_kv,
        ...
        jitted_primitive = jit(
            value_and_grad(
                lambda q, kv, q_token, kv_token: grad_func(
                    customcall_cross_mha, q, kv, q_token, kv_token, **kwargs),
                (0, 1)))

        jitted_reference = jit(
            value_and_grad(
                lambda q, kv, q_token, kv_token: grad_func(
                    jax_cross_mha, q.astype(jnp.float32), kv.astype(
                        jnp.float32), q_token, kv_token, **kwargs), (0, 1)))

        primitive_out, (primitive_dq, primitive_dkv) = jitted_primitive(
            self.q, self.kv, self.q_token, self.kv_token)

        reference_out, (reference_dq, reference_dkv) = jitted_reference(
            self.q, self.kv, self.q_token, self.kv_token)
        ...
        valid_primitive_dq, invalid_primitive_dq = jnp.split(
            primitive_dq, (self.q_valid_len, ), axis=1)
        valid_reference_dq, invalid_reference_dq = jnp.split(
            reference_dq, (self.q_valid_len, ), axis=1)

        valid_primitive_dkv, invalid_primitive_dkv = jnp.split(
            primitive_dkv, (self.kv_valid_len, ), axis=1)
        valid_reference_dkv, invalid_reference_dkv = jnp.split(
            reference_dkv, (self.kv_valid_len, ), axis=1)

        # dQ
        np.testing.assert_allclose(jnp.asarray(valid_primitive_dq[:, :, :, :],
                                               np.float32),
                                   jnp.asarray(valid_reference_dq[:, :, :, :],
                                               np.float32),
                                   rtol=1e-4,
                                   atol=1e-5)
        ...
        # dK
        np.testing.assert_allclose(
            jnp.asarray(valid_primitive_dkv[:, :, 0, :, :], np.float32),
        ...
        # dV
        np.testing.assert_allclose(
            jnp.asarray(valid_primitive_dkv[:, :, 1, :, :], np.float32),
        ...
        # Padded part should be 0s
        assert jnp.allclose(invalid_primitive_dq,
                            jnp.zeros_like(invalid_primitive_dq))
        assert jnp.allclose(invalid_primitive_dkv,
                            jnp.zeros_like(invalid_primitive_dkv))
